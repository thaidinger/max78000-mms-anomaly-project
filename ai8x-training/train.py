...
def _safe_log_token(value):
    """MMS NOTE: keep model/dataset labels readable and safe as folder names."""
    return re.sub(r'[^A-Za-z0-9_.-]+', '-', str(value)).strip('-')


def _training_experiment_name(args):
    """MMS NOTE: append useful run identity after the timestamp in the log folder."""
    parts = []
    if args.name:
        parts.append(args.name)
    parts.extend([getattr(args, 'cnn', None) or getattr(args, 'arch', None), args.dataset])
    return '___'.join(_safe_log_token(part) for part in parts if _safe_log_token(part))
...
def main():
    ...
    if local_rank <= 0:  # not DistributedDataParallel or rank 0
        experiment_name = _training_experiment_name(args)
    ...
    camera_test_loader = None
    if local_rank < 1 and args.dataset.startswith('MMS') and not args.evaluate:
        camera_dataset_fn = locate('datasets.mms.mms_camera_test_get_dataset')
        camera_root = os.path.join(os.path.expanduser(args.data), 'MMSCameraTest')
        if camera_dataset_fn is not None and os.path.isdir(camera_root):
            camera_test_dataset = camera_dataset_fn((os.path.expanduser(args.data), args))
            camera_test_loader = torch.utils.data.DataLoader(
                camera_test_dataset, batch_size=args.batch_size, shuffle=False,
                num_workers=args.workers, pin_memory=args.device == 'cuda'
            )
            msglogger.info('Camera test dataset size: %d', len(camera_test_dataset))
        ...
        if run_validation or run_nas_validation:
            ...
            if args.obj_detection:
                ...
                if args.num_classes > 5 and not args.dr:
                    stats[1]['Top5'] = top5
                if getattr(args, 'image_level_stats', None):
                    stats[1].update(args.image_level_stats)

            distiller.log_training_progress(stats, None, epoch, steps_completed=0, total_steps=1,
                                            log_freq=1, loggers=all_tbloggers)

            if args.dataset.startswith('MMS') and test_loader is not None:
                msglogger.info('--- test (epoch=%d) -------------', epoch)
                test_top1, test_top5, test_loss, test_mAP = _validate(
                    test_loader, model, criterion, [pylogger], args, epoch
                )
                if args.obj_detection:
                    test_stats = ('Performance/Test/', OrderedDict([('Loss', test_loss),
                                                                    ('mAP', test_mAP)]))
                elif args.regression:
                    test_stats = ('Performance/Test/', OrderedDict([('Loss', test_loss),
                                                                    ('MSE', test_top1)]))
                    ...
                    test_stats = ('Performance/Test/', OrderedDict([('Loss', test_loss),
                                                                    ('Top1', test_top1)]))
                    if args.num_classes > 5 and not args.dr:
                        test_stats[1]['Top5'] = test_top5
                    if getattr(args, 'image_level_stats', None):
                        test_stats[1].update(args.image_level_stats)

                distiller.log_training_progress(test_stats, None, epoch, steps_completed=0,
                                                total_steps=1, log_freq=1,
                                                loggers=all_tbloggers)

                _mms_log_tb_images_from_loader(
                    tflogger, test_loader, model, args, epoch, 'Test/MMS', args.tb_image_samples,
                    damaged_only=True
                )

            if camera_test_loader is not None:
                msglogger.info('--- camera test image metrics (epoch=%d) -------------', epoch)
                camera_stats = _mms_image_level_only(camera_test_loader, model, args)
                msglogger.info('==> Camera ImageAcc: %.3f    ImageRecall: %.3f    '
                               'ImagePrecision: %.3f    ImageThresh: %.3f\n',
                               camera_stats['ImageAcc'],
                               camera_stats['ImageRecall'],
                               camera_stats['ImagePrecision'],
                               camera_stats['ImageThresh'])
                distiller.log_training_progress(
                    ('Performance/CameraTest/', camera_stats), None, epoch,
                    steps_completed=0, total_steps=1, log_freq=1, loggers=all_tbloggers
                )
                _mms_log_tb_images_from_loader(
                    tflogger, camera_test_loader, model, args, epoch, 'CameraTest/MMS',
                    args.tb_image_samples
                )
...
MMS_MAP_LOSS_KEY = 'MMS Map Loss'
MMS_FOCAL_LOSS_KEY = 'MMS Focal Loss'
MMS_IMAGE_LOSS_KEY = 'MMS Image Loss'
MMS_IMAGE_LOSS_WEIGHT = 0.2
MMS_FOCAL_LOSS_WEIGHT = 1.0
MMS_FOCAL_GAMMA = 2.0


def _split_mms_output(output):
    """Return map logits and optional image logits from MMS models."""
...
def train(train_loader, model, criterion, optimizer, epoch,
    ...
    for train_step, (inputs, target_temp) in enumerate(train_loader):
        ...
        output, image_logits = _split_mms_output(output)
        if args.out_fold_ratio != 1:
            output = ai8x.unfold_batch(output, args.out_fold_ratio)

        mms_map_loss = None
        mms_focal_loss = None
        if not args.obj_detection and _is_binary_grid_output(output, target, args):
            loss, mms_map_loss, mms_focal_loss = _mms_map_loss(output, target)
        else:
            loss = criterion(output, target)
        mms_image_loss = None
        if not args.obj_detection and _is_binary_grid_output(output, target, args):
            mms_image_loss = _mms_image_level_loss(output, target, image_logits)
            loss = loss + MMS_IMAGE_LOSS_WEIGHT * mms_image_loss
        ...
        if mms_map_loss is not None:
            if MMS_MAP_LOSS_KEY not in losses:
                losses[MMS_MAP_LOSS_KEY] = tnt.AverageValueMeter()
            losses[MMS_MAP_LOSS_KEY].add(mms_map_loss.item())
        if mms_focal_loss is not None:
            if MMS_FOCAL_LOSS_KEY not in losses:
                losses[MMS_FOCAL_LOSS_KEY] = tnt.AverageValueMeter()
            losses[MMS_FOCAL_LOSS_KEY].add(mms_focal_loss.item())
        if mms_image_loss is not None:
            if MMS_IMAGE_LOSS_KEY not in losses:
                losses[MMS_IMAGE_LOSS_KEY] = tnt.AverageValueMeter()
            losses[MMS_IMAGE_LOSS_KEY].add(mms_image_loss.item())
...
def _is_binary_grid_output(output, target, args):
    """Return true for MMS-style binary anomaly-grid outputs."""
    return args.dataset.startswith('MMS') and output.dim() == 4 and target.dim() == 3 \
        and output.shape[1] == 2


def _mms_per_image_weights(target):
    """Balance each MMS mask independently so small defects are not underweighted."""
    target = target.float()
    pos = target
    neg = 1. - target

    pos_count = pos.flatten(start_dim=1).sum(dim=1)
    neg_count = neg.flatten(start_dim=1).sum(dim=1).clamp_min(1.)
    has_pos = pos_count > 0

    pos_norm = torch.where(has_pos, 0.5 / pos_count.clamp_min(1.),
                           torch.zeros_like(pos_count))
    neg_norm = torch.where(has_pos, 0.5 / neg_count, 1. / neg_count)

    return pos * pos_norm[:, None, None] + neg * neg_norm[:, None, None]


def _mms_map_loss(output, target):
    """Per-image balanced CE plus focal loss for MMS anomaly maps."""
    ce_map = nn.functional.cross_entropy(output, target, reduction='none')
    weights = _mms_per_image_weights(target)
    balanced_ce = (ce_map * weights).flatten(start_dim=1).sum(dim=1).mean()

    probs = nn.functional.softmax(output, dim=1)
    pt = probs.gather(1, target.unsqueeze(1)).squeeze(1).clamp(min=1e-6, max=1. - 1e-6)
    focal_map = ((1. - pt) ** MMS_FOCAL_GAMMA) * ce_map
    focal_loss = (focal_map * weights).flatten(start_dim=1).sum(dim=1).mean()

    return balanced_ce + MMS_FOCAL_LOSS_WEIGHT * focal_loss, balanced_ce, focal_loss


def _mms_image_level_loss(output, target, image_logits=None):
    """Classify good/damaged using aux logits, or pooled map logits as fallback."""
    if image_logits is None:
        image_logits = output.mean(dim=(2, 3))
    image_target = target.flatten(start_dim=1).any(dim=1).long()
    return nn.functional.cross_entropy(image_logits, image_target)


def _binary_image_level_metrics(scores, labels):
    """Compute image-level good/damaged metrics from anomaly scores."""
    if scores.numel() == 0:
        return OrderedDict()

    labels = labels.bool()
    thresholds = torch.linspace(0, 1, 101, device=scores.device)
    ...
    for threshold in thresholds:
        ...
        tp = torch.logical_and(pred, labels).sum().float()
        tn = torch.logical_and(~pred, ~labels).sum().float()
        fp = torch.logical_and(pred, ~labels).sum().float()
        fn = torch.logical_and(~pred, labels).sum().float()

        precision = tp / (tp + fp + 1e-12)
        recall = tp / (tp + fn + 1e-12)
        f1 = 2 * precision * recall / (precision + recall + 1e-12)
        acc = (tp + tn) / (tp + tn + fp + fn + 1e-12)
        good_acc = tn / (tn + fp + 1e-12)
        damaged_acc = tp / (tp + fn + 1e-12)
        balanced_acc = 0.5 * (good_acc + damaged_acc)

        if balanced_acc.item() > best['balanced_acc'] \
           or (balanced_acc.item() == best['balanced_acc'] and f1.item() > best['f1']):
            best = {
                'f1': f1.item(),
                'acc': acc.item(),
    ...
    one_cell_threshold = 1. / 1024.
    pred_one_cell = scores >= one_cell_threshold
    one_cell_acc = (pred_one_cell == labels).float().mean().item()

    return OrderedDict([
        ('ImageAcc', 100. * best['acc']),
        ('ImageF1', 100. * best['f1']),
        ('ImageBalancedAcc', 100. * best['balanced_acc']),
        ('ImagePrecision', 100. * best['precision']),
        ('ImageRecall', 100. * best['recall']),
        ('ImageThresh', best['threshold']),
        ('ImageAccOneCell', 100. * one_cell_acc),
    ])


def _mms_image_level_only(data_loader, model, args):
    """Evaluate only image-level MMS good/damaged metrics."""
    image_scores = []
    image_labels = []
    ...
    with torch.no_grad():
        for inputs, target in data_loader:
            ...
            output = model(inputs)
            output, _ = _split_mms_output(output)
            if args.out_fold_ratio != 1:
                output = ai8x.unfold_batch(output, args.out_fold_ratio)
            if args.act_mode_8bit:
                output /= 128.
            ...
            anomaly_probs = nn.functional.softmax(output, dim=1)[:, 1, :, :]
            image_scores.append(anomaly_probs.mean(dim=(1, 2)).detach().cpu())
            image_labels.append(target.flatten(start_dim=1).any(dim=1).detach().cpu())

    if not image_scores:
        return OrderedDict()

    return _binary_image_level_metrics(torch.cat(image_scores), torch.cat(image_labels))


def _mms_should_log_tb_images(args, epoch, tflogger):
    """Return true when MMS test/camera-test image panels should be logged."""
    return tflogger is not None and args.dataset.startswith('MMS') and epoch >= 0 \
        and args.tb_image_samples > 0 and args.tb_image_freq > 0 \
        and epoch % args.tb_image_freq == 0


def _mms_rgb_from_input(input_tensor):
    """Convert a normalized CHW input tensor to displayable RGB using per-image min/max."""
...
def _mms_mask_rgb(mask, size):
    """Convert a binary mask to RGB red-on-black."""
    mask = mask.detach().cpu().float().unsqueeze(0).unsqueeze(0)
    mask = nn.functional.interpolate(mask, size=size, mode='nearest').squeeze(0).squeeze(0)
    return torch.stack((mask, torch.zeros_like(mask), torch.zeros_like(mask))).clamp(0., 1.)


def _mms_heat_rgb(heat, size):
    """Convert anomaly probability heatmap to a simple black-red-yellow RGB map."""
    heat = heat.detach().cpu().float().unsqueeze(0).unsqueeze(0)
    heat = nn.functional.interpolate(heat, size=size, mode='bilinear',
                                     align_corners=False).squeeze(0).squeeze(0)
    heat = heat.clamp(0., 1.)
    return torch.stack((heat, heat * heat, torch.zeros_like(heat))).clamp(0., 1.)


def _mms_log_tb_images(tflogger, tag, inputs, target, output, epoch, start_index, max_samples):
    """Log input/target/prediction/overlay panels for MMS examples."""
    if max_samples <= 0:
        return 0

    anomaly_probs = nn.functional.softmax(output.detach(), dim=1)[:, 1, :, :]
    logged = 0
    for index in range(min(inputs.size(0), max_samples)):
        image = _mms_rgb_from_input(inputs[index])
        height, width = image.shape[1:]
        target_rgb = _mms_mask_rgb(target[index], (height, width))
        pred_rgb = _mms_heat_rgb(anomaly_probs[index], (height, width))
        overlay = (0.65 * image + 0.55 * pred_rgb).clamp(0., 1.)
        panel = torch.cat((image, target_rgb, pred_rgb, overlay), dim=2)
        sample_id = start_index + index
        tflogger.tblogger.writer.add_image(
            f'{tag}/sample_{sample_id:02d}_input_target_pred_overlay',
            panel, epoch
        )
        logged += 1
    return logged
...
def _mms_log_tb_images_from_loader(
        tflogger, data_loader, model, args, epoch, tag, max_samples, damaged_only=False):
    """Log a fixed number of MMS image panels from a complete loader."""
    if not _mms_should_log_tb_images(args, epoch, tflogger) or data_loader is None:
        return 0
    ...
    with torch.no_grad():
        for inputs, target in data_loader:
            if damaged_only:
                keep = target.flatten(start_dim=1).any(dim=1)
                if not keep.any():
                    continue
                inputs = inputs[keep]
                target = target[keep]
            ...
            if args.act_mode_8bit:
                ...
                for _, module in model.named_modules():
                    if hasattr(module, 'wide') and module.wide:
                        output /= 128.
            logged += _mms_log_tb_images(
                tflogger, tag, inputs, target, output, epoch, logged, max_samples - logged
            )
...
def _validate(data_loader, model, criterion, loggers, args, epoch=-1, tflogger=None):
    ...
    image_scores = []
    image_labels = []
    args.image_level_stats = OrderedDict()
    # MMS image panels are logged explicitly from the test and camera-test loaders.
    tb_images_remaining = 0
    tb_images_logged = 0
        ...
        for validation_step, (inputs, target_temp) in enumerate(data_loader):
            ...
            if not args.obj_detection and not args.kd_relationbased:
                if len(output.data.shape) <= 2 or args.regression:
                    ...
                    classerr.add(output.data.permute(0, 2, 3, 1).flatten(start_dim=0, end_dim=2),
                                 target.flatten())
                    if _is_binary_grid_output(output, target, args):
                        anomaly_probs = nn.functional.softmax(output, dim=1)[:, 1, :, :]
                        image_scores.append(anomaly_probs.mean(dim=(1, 2)).detach().cpu())
                        image_labels.append(target.flatten(start_dim=1).any(dim=1).detach().cpu())
                        if tb_images_remaining > 0:
                            logged = _mms_log_tb_images(
                                tflogger, 'Validation/MMS', inputs, target, output, epoch,
                                tb_images_logged, tb_images_remaining
                            )
                            tb_images_logged += logged
                            tb_images_remaining -= logged
                if args.display_confusion:
                    if _is_binary_grid_output(output, target, args):
                        confusion.add(
                            output.data.permute(0, 2, 3, 1).flatten(start_dim=0, end_dim=2),
                            target.flatten()
                        )
                    else:
            ...
            if steps_completed % args.print_freq == 0 or steps_completed == total_steps:
                ...
                if args.kd_relationbased:
                    ...
                    if image_scores:
                        running_scores = torch.cat(image_scores)
                        running_labels = torch.cat(image_labels)
                        stats[1].update(_binary_image_level_metrics(running_scores,
                                                                     running_labels))
                if MMS_MAP_LOSS_KEY in losses:
                    stats[1][MMS_MAP_LOSS_KEY] = losses[MMS_MAP_LOSS_KEY].mean
                if MMS_FOCAL_LOSS_KEY in losses:
                    stats[1][MMS_FOCAL_LOSS_KEY] = losses[MMS_FOCAL_LOSS_KEY].mean
    ...
    if image_scores:
        args.image_level_stats = _binary_image_level_metrics(torch.cat(image_scores),
                                                             torch.cat(image_labels))
        msglogger.info('==> ImageAcc: %.3f    ImageF1: %.3f    ImageBalancedAcc: %.3f    '
                       'ImageThresh: %.3f\n',
                       args.image_level_stats['ImageAcc'],
                       args.image_level_stats['ImageF1'],
                       args.image_level_stats['ImageBalancedAcc'],
                       args.image_level_stats['ImageThresh'])
