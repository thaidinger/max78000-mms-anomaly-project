...
def pre_qat(model, train_loader, args, qat_policy):
    ...
    """
    # CODEx-MPS-QAT-STATS: torch.histc() can request invalid huge buffers on MPS during
    # activation histogram collection. Keep normal training on MPS, but collect QAT stats on CPU.
    qat_stats_device = args.device
    if qat_stats_device == 'mps':
        model.to('cpu')
        args.device = 'cpu'
